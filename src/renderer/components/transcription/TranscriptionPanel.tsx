...
import {
  useVisualIndexStore,
  VisualIndexItem,
} from "../../stores/visual-index.store";
...
function TranscriptMessage({ item }: TranscriptMessageProps) {
  ...
  return (
    <div className="flex flex-col gap-[8px]">
      ...
      {/* Spanish translation for "Them" messages */}
      {!isMe && item.translation && (
        <div className="flex items-start gap-[6px] mt-[-2px]">
          <span className="shrink-0 text-[11px] font-semibold text-[#2d8cff] uppercase tracking-wide leading-[20px]">
            ES
          </span>
          <p className="text-[14px] text-[#1a56db] leading-[22px] italic">
            {item.translation}
          </p>
        </div>
      )}
...
export function TranscriptionPanel() {
  const { items, enabled, pendingMic, pendingSystemAudio } =
    useTranscriptionStore();
  ...
  const totalItemCount =
    mergedItems.length + (pendingMic ? 1 : 0) + (pendingSystemAudio ? 1 : 0);
          ...
          <>
            {mergedItems.map((item) =>
              item.type === "transcript" ? (
                <TranscriptMessage
                  key={(item.data as TranscriptItem).id}
                  item={item.data as TranscriptItem}
                />
              ) : (
                <VisualAnalysisEntry
                  key={(item.data as VisualIndexItem).id}
                  item={item.data as VisualIndexItem}
                />
              ),
            )}
            ...
            {pendingSystemAudio && (
              <PendingMessage text={pendingSystemAudio} source="system_audio" />
            )}
          </>
        )}
      </div>
